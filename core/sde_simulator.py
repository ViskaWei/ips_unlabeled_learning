...
class SDESimulator:
    ...
    def compute_interaction_gradient(self, X: np.ndarray) -> np.ndarray:
        """Compute mean-field interaction gradient for each particle.
        ...
        """
        N, d = X.shape
        grad_interaction = np.zeros_like(X)

        for i in range(N):
            for j in range(N):
                if i != j:
                    diff = X[i] - X[j]  # shape (d,)
                    r = np.linalg.norm(diff)
                    if r > 1e-10:  # avoid division by zero
                        # grad_Phi w.r.t. X_i is (dPhi/dr) * (diff/r)
                        grad_r = self.Phi.gradient(r)
                        grad_interaction[i] += grad_r * (diff / r)
